...
!             CLDFLAG,NRANDO,IRAN,L3RG,NICA,JCOUNT)
      ...
      ['clear sky - no clouds','avg cloud cover', 'avg cloud cover^3/2', &
       'ICAs - avg direct beam', 'ICAs - random N ICAs', &
       'QCAs - midpt of bins','QCAs - avg clouds in bins', &
       'ICAs - use all ICAs***']
...
!--------------------key params sent to CLOUD_JX-------------------------
      ...
      integer                    :: CLDFLAG,NRANDO,IRAN,L3RG
      integer                    :: NICA,JCOUNT
...
!   from tropical T319 ECMWF atmosphere used in UCI CTM.
      ...
      do ICLD=1,CLDFLDS
       ...
       do L = LWEPAR,1,-1
        ...
        if (CLDFRW(L) .gt. 0.001d0) then
          write(6,'(i5,f10.4,1p,2e10.2)')  &
                     J,CLDFRW(L),CLDLWCW(L),CLDIWCW(L)
        endif
       enddo
...
!  15HNO3      PHOTON    NO2       OH        1.000 mapped to FJX:  13 HNO3
      ...
      enddo   !CLDFLAG
       ...
       do L=34,1,-1
        ZKM = 1.d-5*ZZZ(L)
        write(6,'(i3,f6.1,1p,72e9.2)') L,ZKM,(ZPJCLD(L,K,J),K=1,8)
       enddo
       enddo
        write(6,'(9x,9i9)') (K, K=1,9)
       endif

! print order
!    1   clear sky - no clouds
!    2   Avg cloud cover
!    3   Avg cloud cover^3/2
!    4   ICAs - Avg direct beam
!    5   ICAs - random N ICAs
!    6   QCAs - all, from mid-pt of bins
!    7   QCAs - all, avg clouds in Q bins
...
! accumulate errors for each cloud field
       ...
       enddo
       if (ICLD .le. 3) then
          write(6,'(5a)') ' error = ln(J/J-exact) L=1  &  L=34   ',  &
                    (TITJX(J),J=1,4)
          write(6,'(a11,4f7.2,5x,4f7.2,5x,4f7.2)') &
           (TITERR(I),(ERRJ(I,J),J=1,4),(ERRJ(I+7,J),J=1,4),         &
                 (ERRJ(I+14,J),J=1,4), I=1,7)
       endif
      enddo ! ICLD
       ...
       enddo
       enddo
       write(6,'(a,2i5)') ' no. atmos / L3RG',ICLD-1,L3RG
